...
**Version 23.0, Last Update: 18 Mar 2024**
...
It is a good exercise to derive eq. (7) by yourself. A good aerospace engineer should be able to do this calculation in a pub, in the middle of the night, after having consumed too much beer, on the back of an envelope. There are two parameters in this equation, the ground density $\rho_0=\rho(h=0)$ and the atmospheric scale height $H$, which is an assembly of other constants (see slide 9). What is the meaning of $H$? First note that the argument of an exponential function must be unitless, a pure number. Then insert $h=H$ into the equation. The result is $\rho(h=H)=\frac{\rho_0}{e}$, where $e = 2.71828$ is the Euler number. So $H$ is the altitude at which the density has decreased to $1/e$ times the value on the ground. Note that the given values for $\rho_0$ and $H$ are not the real values on ground but values that give the best overall fit for $0\,\mathrm{km} \le h\le 122\,\mathrm{km}$. 
...
In the following calculations, try to check every step and ask yourself "Why are we doing this step?" If you can't answer this question right now, don't worry. Just follow all steps until slide 20. Then go back and answer this "Why are we doing this step?" question.
...
Again, in the following calculations, try to check every step and ask yourself "Why are we doing this step?" If you can't answer this question right now, don't worry. Just follow all steps until slide 33. Then go back and answer this "Why are we doing this step?" question.
