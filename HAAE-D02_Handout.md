...
For atmospheric entry calculations, the entry velocity $V_0$ of an entry vehicle is the most important parameter (together with the entry angle $\gamma_0$). In this sense, this week's lecture is not intended as a crash course (forgive me the word) into spaceflight mechanics. We will only cover the subjects that are necessary to calculate $V_0$. I'm sure you already know that the atmosphere does not have a fixed interface to the vacuum of space (although the legal definition for "space" is the altitude of 100 km, but here we don't care about legal stuff, just about physics). In the most simple model, starting with a ground density of $\rho_0$ at the surface, the atmosphere gets exponentially thinner with altitude $h$, i.e. $\propto e^{-h}$, where $\propto$ reads "proportional to". Nevertheless, in practice, we get the best analytical results, if we assume that there is no atmosphere for $h \gtrapprox$ 122 km (note: not 100 km!) and then the exponential atmosphere for $h \lessapprox$ 122 km. So we "only" have to calculate the entry vehicle velocity $V_0$ at this altitude of $h_0 =$ 122 km. Unfortunately (or fortunately?), this requires some basic knowledge on spaceflight dynamics, especially elliptic orbits and hyperbolic trajectories (since those are the conic sections / entry trajectories that entry vehicles must use), spheres of influences, and orbital maneuvers (to change the vehicle's trajectory). At the end, we will look at three examples that hopefully bring life to the theory that some of you may consider boring.

Note for bullet 3: In the so called two-body problem (e.g. Earth and spacecraft), an ellipse is a closed repeating path. An hyperbola is an open path that - mathematically - arrives from infinity and flies back into infinity. (Of course, infinity does not exist in reality, but the difference between very veery veeery far away and infinitely far away is practically negligible.) It is better to call open paths trajectories, since the word *orbit* (and *orbiting* and *orbiter*) sounds "repeating".

Note for bullet 4: We use the gravitational parameter $\mu$ of a planet, since the mass $M$ of a planet is not well known. Also - in comparison to other constants, like the speed of light - the gravitational constant $G$ is not very well known. Only 6 relevant digits are not accurate enough for spaceflight. Their product however, $\mu = GM$, is known with a very high accuracy. Why? Because this is the *only* parameter that "acts" onto spacecraft, and so it can be measured with a very high accuracy. If we know $\mu$, we do not have to care whether $G$ is a bit smaller and $M$ is a bit larger or vice versa, only their product, $\mu$, is important.
